...
// あたらしいTDをつくるには、以下のようにする
// 1. enumにTD_??を追加する
// 2. tap_dance_actions[]にACTION_TAP_DANCE_FN_ADVANCED(NULL, ?_finished, ?_reset)を追加する
// 3. static tap ?tap_stateを追加する
// 4. void ?_finished()を追加する
// 5. void ?_reset()を追加する
...
// utility function to repeat a key x times
...
// utility function to double shift
...
void p_finished (tap_dance_state_t *state, void *user_data) {
    ...
    switch (ptap_state.state) {
        ...
        case TRIPLE_SINGLE_TAP:
            ...
            break;
        case MORE_TAP:
            repeat_key_x_time(KC_P, state->count);
            break;
        default:
            break;
    }
...
void slash_finished (tap_dance_state_t *state, void *user_data) {
    ...
    switch (slash_tap_state.state) {
        ...
        case TRIPLE_SINGLE_TAP:
            ...
            break;
        case MORE_TAP:
            repeat_key_x_time(KC_SLASH, state->count);
            break;
        default:
            break;
    }
...
// keycode definitions for tap dance actions
enum {
    TD_DOUBLE_SHIFT = 0,
    TD_B,
    TD_C,
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [0] = LAYOUT_universal(
    ...
    XXXXXXX      , KC_LNG1      , KC_LNG2      , LT(3,KC_ESC) , LT(2,KC_SPC) , LT(1,KC_TAB) ,       LT(1,KC_ENTER) , LT(2,KC_BSPC) , XXXXXXX      , XXXXXXX      , XXXXXXX        , XXXXXXX         ,
  ),
  ...
  [2] = LAYOUT_universal(
    KC_F1   , KC_F2   , KC_F3   , KC_F4   , KC_INS   ,                         KC_PGUP , KC_HOME  , KC_END  , KC_PGUP , KC_GRV  ,
    KC_F5   , KC_F6   , KC_F7   , KC_F8   , KC_SCRL  ,                         KC_LEFT , KC_DOWN  , KC_UP   , KC_RGHT , KC_QUOT ,
    KC_F9   , KC_F10  , KC_F11  , KC_F12  , KC_PAUSE ,                         KC_PGDN , KC_MINUS , KC_EQL  , KC_PGDN , KC_BSLS ,
    KC_F13  , KC_F14  , KC_F15  , XXXXXXX , XXXXXXX  , TO(0) ,       TO(0)   , KC_DEL  , XXXXXXX  , XXXXXXX , XXXXXXX , XXXXXXX ,
  ),

  [3] = LAYOUT_universal(
    KBC_RST  , XXXXXXX , SSNP_HOR , SSNP_VRT , AML_TO  ,                          A(KC_F4)             , A(KC_LEFT) , A(KC_RIGHT) , XXXXXXX , KC_GRV  ,
    KBC_SAVE , XXXXXXX , CPI_D100 , CPI_I100 , QK_BOOT ,                          C(KC_W)              , KC_BTN1    , KC_BTN3     , KC_BTN2 , KC_QUOT ,
    XXXXXXX  , XXXXXXX , SCRL_DVD , SCRL_DVI , EE_CLR  ,                          C(S(KC_T))           , KC_PGUP    , KC_PGDN     , XXXXXXX , KC_BSLS ,
    XXXXXXX  , XXXXXXX , XXXXXXX  , XXXXXXX  , KC_SPC  , TO(0) ,       TO(0)   ,  TD(TD_DOUBLE_SHIFT)  , XXXXXXX    , XXXXXXX     , XXXXXXX , XXXXXXX ,
  ),
};
// clang-format on
